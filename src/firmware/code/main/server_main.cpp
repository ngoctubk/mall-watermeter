...
esp_err_t auth_handler(httpd_req_t *req) {
    ...
    size_t buf_len = httpd_req_get_hdr_value_len(req, "X-Meter-Secret") + 1;
    if (buf_len > 1 && buf_len <= sizeof(header_value)) {
        if (httpd_req_get_hdr_value_str(req, "X-Meter-Secret", header_value, buf_len) == ESP_OK) {
            if (strcmp(header_value, "Secret") == 0) {
                return ESP_OK;
            }
        }
    }
